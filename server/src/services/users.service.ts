...
import { calculateDaysWorked } from '../utils/calculateDaysWorked';
...
const USER_SORT_FIELDS = ['id', 'name', 'department', 'dateHired'] as const;
type UserSortField = (typeof USER_SORT_FIELDS)[number];

export function getUsers(req: Request, users: User[]) {
  const query = req.query as Record<string, unknown>;
  
  const usersDaysHired = users.map(user => ({
    ...user, daysWorked: calculateDaysWorked(user.dateHired)
  }));
  ...
  const sorted = sortCollection(usersDaysHired, field, direction, 'id');
